...
class List(unittest.TestCase):
    def setUp(self):
        self.filename = Request().dirname() + "/Document/Agent_Manage/list.json"
    ...
    def get_response(self, serial):
        ...
        :return: 获取接口返回
        """
        url = Request().get_manage_url(serial=serial, filename=self.filename, apiname=self.apiname)
    ...
    def test_null_search(self):
        ...
        for key, value in casenum.items():
            ...
            print(response.text)
            json_result = json.loads(re.sub('\'', '\"', response.text))
